...
if (!$val) {
  ...
  exit();
} elseif ($_POST["user_id"] == "testuser" ){
  // $_SESSION = array();
  // $_SESSION["session_id"] = session_id();
  $_SESSION["user_id"] = $val["user_id"];
  header("Location:../HTML/product_manager.html");
} else {
  $_SESSION = array();
  $_SESSION["session_id"] = session_id();
  //以下is_adminとuser_idは管理者がログイン中かそうでないか判断する時に使える。
  $_SESSION["is_admin"] = $val["is_admin"];
  $_SESSION["user_id"] = $val["user_id"];
  $_SESSION["user_id"] = $val["user_id"];
  $_SESSION["id"] = $val["id"];
  header("Location:../HTML/product_login.html");
  exit();
}
